...
def get_all_data_loaders(conf):
    ...
    #     # images are from pytorch / train_all
    ...
    if conf['use_cloth']:
        if data_name == "prcc":
            print("------Using PRCC Cloth Datasets------")
            train_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'train'), batch_size, True, data_name,
                                                    new_size_a, height, width, num_workers, True)
            test_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'test', 'C'), batch_size, False, data_name,
                                                   new_size_a, height, width, num_workers, False)
            train_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'train'), batch_size, True, data_name,
                                                    new_size_b, height, width, num_workers, True)
            test_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'test', 'C'), batch_size, False, data_name,
                                                   new_size_b, height, width, num_workers, False)
        elif data_name == 'ltcc' or data_name == 'vc' or data_name == 'celeb' or data_name == 'celeb_light':
            print("------Using ", data_name, " Cloth Datasets------")
            train_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'train'), batch_size, True, data_name,
                                                    new_size_a, height, width, num_workers, True)
            test_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'query'), batch_size, False, data_name,
                                                   new_size_a, height, width, num_workers, False)
            train_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'train'), batch_size, True, data_name,
                                                    new_size_b, height, width, num_workers, True)
            test_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'query'), batch_size, False, data_name,
                                                   new_size_b, height, width, num_workers, False)
        else:
            print("Undefined", data_name, ", please checke the CONFIG file")
            raise RuntimeError
...
def get_data_loader_folder(input_folder, batch_size, train, data_name, new_size=None,
                           height=256, width=128, num_workers=4, crop=True):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.485, 0.456, 0.406),
                                           (0.229, 0.224, 0.225))]
    ...
    transform_list = [transforms.RandomHorizontalFlip()] + transform_list if train else transform_list
    transform = transforms.Compose(transform_list)
    dataset = ReIDFolder(input_folder, transform=transform, dataset_name=data_name)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=num_workers)
    return loader

...
from reIDmodel import ft_net, ft_netAB, PCB, ACID
...
class DGNet_Trainer(nn.Module):
    def __init__(self, hyperparameters, gpu_ids=[0]):
        ...
        if not 'CA_ALL' in hyperparameters.keys():
            self.CA_ALL = False
        else:
            self.CA_ALL = hyperparameters['CA_ALL']
        ...
        if hyperparameters['ID_style']=='PCB':
            ...
            self.id_a = ft_netAB(ID_class, stride = hyperparameters['ID_stride'], norm=hyperparameters['norm_id'], pool=hyperparameters['pool'], circle=hyperparameters['circle'], ibn=hyperparameters['ibn'])
            # print model
            print(self.id_a)
        elif hyperparameters['ID_style'] == 'ACID':
            self.global_att = True
            if not 'CA' in hyperparameters.keys():
                self.CA = False
            else:
                self.CA = hyperparameters['CA']

            self.soft = hyperparameters['soft']
            if "treshold_channel" in hyperparameters.keys():
                self.threshold = hyperparameters['treshold_channel']
            else:
                self.threshold = 1024

            if self.soft:
                print("---------Using soft Version-----------")
                print("---------channel threshold=", self.threshold, "-----------")
            elif not self.CA:
                print("---------Using normal Version-----------")
            elif self.CA:
                print("---------Using normal full Version-----------")
            else:
                raise ValueError("Please set the correct version")

            self.id_a = ACID(ID_class, stride = hyperparameters['ID_stride'], norm=hyperparameters['norm_id'], pool=hyperparameters['pool'], ca_layer=hyperparameters['CA_layer'], circle=hyperparameters['circle'], ibn=hyperparameters['ibn'], soft=self.soft, th = self.threshold, using_global=self.global_att)
        ...
        if hyperparameters['ID_style']=='PCB':
            ...
            # there is another classifier for ACID
            ignored_params = (list(map(id, self.id_a.classifier1.parameters()))
                              + list(map(id, self.id_a.classifier2.parameters()))
                              + list(map(id, self.id_a.classifierCA.parameters())))
            base_params = filter(lambda p: id(p) not in ignored_params, self.id_a.parameters())
            lr2 = hyperparameters['lr2']
            self.id_opt = torch.optim.SGD([
                {'params': base_params, 'lr': lr2},
                {'params': self.id_a.classifier1.parameters(), 'lr': lr2 * 10},
                {'params': self.id_a.classifier2.parameters(), 'lr': lr2 * 10},
                {'params': self.id_a.classifierCA.parameters(), 'lr': lr2 * 10}
            ], weight_decay=hyperparameters['weight_decay'], momentum=0.9, nesterov=True)
        else:
        ...
        if 'margin' in hyperparameters:
            margin = hyperparameters['margin']
            dist_style = hyperparameters['dist_style']
        else:
            margin = 0.5
            dist_style = 'euclidean'

        self.triplet = TripletLoss(margin=margin, dist_style=dist_style)


        # Circle Loss
        if hyperparameters['circle']:
    ...
    def get_gt(self, gt_label_a, gt_label_b):
        self.gt_a = gt_label_a
        self.gt_b = gt_label_b
    ...
    def forward(self, x_a, x_b, xp_a, xp_b, xc_a, xc_b):
        ...
        # feature, socre
        # p_a,p_b[[feature, score],[feature, score]]

        if self.ACID:
            f_a, p_a, feature_a, causal_a = self.id_a(scale2(x_a), self.gt_a, self.CA)
            f_b, p_b, feature_b, causal_b = self.id_b(scale2(x_b), self.gt_b, self.CA)
        else:
        ...
        # another pose, same id
        if self.ACID:
            fp_a, pp_a, featurep_a, causalp_a = self.id_a(scale2(xp_a), self.gt_a, self.CA)
            fp_b, pp_b, featurep_b, causalp_b = self.id_b(scale2(xp_b), self.gt_b, self.CA)

        else:
            fp_a, pp_a = self.id_a(scale2(xp_a))
            fp_b, pp_b = self.id_b(scale2(xp_b))

        # another cloth and pose, same id
        if self.ACID:
            fc_a, pc_a, featurec_a, causalc_a = self.id_a(scale2(xc_a), self.gt_a, self.CA)
            fc_b, pc_b, featurec_b, causalc_b = self.id_b(scale2(xc_b), self.gt_b, self.CA)

        else:
            fc_a, pc_a = self.id_a(scale2(xc_a))
            fc_b, pc_b = self.id_b(scale2(xc_b))
        ...
        if self.erasing_p > 0:
            ...
            xc_a_re = self.to_re(scale2(xc_a.clone()))
            xc_b_re = self.to_re(scale2(xc_b.clone()))

            if self.ACID:
                _, p_a, feature_a, causal_a = self.id_a(x_a_re, self.gt_a, self.CA)
                _, p_b, feature_b, causal_b = self.id_b(x_b_re, self.gt_b, self.CA)

                # encode the same ID different pose photo
                _, pp_a, featurep_a, causalp_a = self.id_a(xp_a_re, self.gt_a, self.CA)
                _, pp_b, featurep_b, causalp_b = self.id_b(xp_b_re, self.gt_b, self.CA)

                # encode the same ID different cloth and pose photo
                _, pc_a, featurec_a, causalc_a = self.id_a(xc_a_re, self.gt_a, self.CA)
                _, pc_b, featurec_b, causalc_b = self.id_b(xc_b_re, self.gt_b, self.CA)
                ...
                # encode the same ID different cloth and pose photo
                _, pc_a = self.id_a(xc_a_re)
                _, pc_b = self.id_b(xc_b_re)

        if self.ACID:
            return x_ab, x_ba, s_a, s_b, f_a, f_b, p_a, p_b, fp_a, fp_b, pp_a, pp_b, pc_a, pc_b, x_a_recon, x_b_recon, x_a_recon_p, x_b_recon_p,\
                   feature_a, feature_b, causal_a, causal_b, featurep_a, featurep_b, causalp_a, causalp_b, featurec_a, featurec_b, causalc_a, causalc_b
        else:
            return x_ab, x_ba, s_a, s_b, f_a, f_b, p_a, p_b, fp_a, fp_b, pp_a, pp_b, pc_a, pc_b, x_a_recon, x_b_recon, x_a_recon_p, x_b_recon_p

    def gen_update(self, x_ab, x_ba, s_a, s_b, f_a, f_b, p_a, p_b, fp_a, fp_b, pp_a, pp_b, pc_a, pc_b, x_a_recon, x_b_recon, x_a_recon_p, x_b_recon_p, x_a, x_b, xp_a, xp_b, feature_a, feature_b, causal_a, causal_b, featurep_a, featurep_b, causalp_a, causalp_b, featurec_a, featurec_b, causalc_a, causalc_b, l_a, l_b, hyperparameters, iteration, num_gpu):

        # ppa, ppb is the same person
        self.gen_opt.zero_grad()
        self.id_opt.zero_grad()

        now_batch_size = hyperparameters['batch_size']
        ...
        # s_b, f_a
        if self.ACID:
            f_a_recon, p_a_recon, feature_a_recon, causal_a_recon = self.id_a_copy(scale2(x_ba), self.gt_a, self.CA_ALL)
            f_b_recon, p_b_recon, feature_b_recon, causal_b_recon = self.id_b_copy(scale2(x_ab), self.gt_b, self.CA_ALL)
        else:
        ...
        if hyperparameters['teacher_w'] >0 and hyperparameters['teacher'] != "":
            ...
            if hyperparameters['ID_style'] == 'normal':
                ...
                _, p_ba_student = self.id_a(scale2(x_ba_copy))  # s_b, f_a

                if hyperparameters['circle']:
                    p_a_student = log_sm(p_ba_student[0][1])
                else:
                ...
                _, p_ab_student = self.id_b(scale2(x_ab_copy))  # s_a, f_b

                if hyperparameters['circle']:
                    p_b_student = log_sm(p_ab_student[0][1])
                else:
                ...
                if hyperparameters['circle']:
                    # feature
                    s_b_student= p_ab_student[1][0]
                    s_a_student= p_ba_student[1][0]

                    # Teacher feature Size([8, 512])
                    # print("s_a_student.shape=", s_a_student.shape)
                    fnrom_a = torch.norm(s_a_student, p=2, dim=1, keepdim=True)
                    fnrom_b = torch.norm(s_b_student, p=2, dim=1, keepdim=True)

                    s_a_student = s_a_student.div(fnrom_a.expand_as(s_a_student))
                    s_b_student = s_b_student.div(fnrom_b.expand_as(s_b_student))

                    loss_B = self.id_criterion(p_ba_student[1][1], l_b) \
                             + self.id_criterion(p_ab_student[1][1], l_a)
                    Loss_B_circle = self.criterion_circle(*convert_label_to_similarity(s_b_student, l_b))/now_batch_size \
                                    + self.criterion_circle(*convert_label_to_similarity(s_a_student, l_a))/now_batch_size
                    loss_B = loss_B + hyperparameters['circle_w'] * Loss_B_circle
                else:
                ...
                # normal teacher-student loss
                # BA -> LabelA(smooth) + LabelB(batchB)
                _, p_ba_student, _, _ = self.id_a(scale2(x_ba_copy), self.gt_a, self.CA_ALL)  # s_b, f_a
                ...
                with torch.no_grad():
                    p_a_teacher = predict_label(self.teacher_model, scale2(x_ba_copy),
                                                num_class=hyperparameters['ID_class'], alabel=l_a, slabel=l_b,
                                                teacher_style=hyperparameters['teacher_style'])

                # (Batch_size * Class_num)
                # print("p_a_student.shape=", p_a_student.shape)
                # print("p_a_teacher.shape=", p_a_teacher.shape)
                # _, tIndex = p_a_teacher.max(1)
                # _, sIndex = p_a_student.max(1)


                self.loss_teacher = self.criterion_teacher(p_a_student, p_a_teacher) / p_a_student.size(0)

                _, p_ab_student, _, _ = self.id_b(scale2(x_ab_copy), self.gt_b, self.CA_ALL)  # s_a, f_b
                ...
                with torch.no_grad():
                    p_b_teacher = predict_label(self.teacher_model, scale2(x_ab_copy),
                                                num_class=hyperparameters['ID_class'], alabel=l_b, slabel=l_a,
                                                teacher_style=hyperparameters['teacher_style'])

                # _, tIndex2 = p_b_teacher.max(1)
                # _, sIndex2 = p_b_student.max(1)

                self.loss_teacher += self.criterion_teacher(p_b_student, p_b_teacher) / p_b_student.size(0)

                # print("Teacher_A:", tIndex)
                # print("Label___A:", self.gt_a)
                # print("Student_A:", sIndex)
                #
                # print("Teacher_B:", tIndex2)
                # print("Label___B:", self.gt_b)
                # print("Student_B:", sIndex2)
        ...
        if hyperparameters['ID_style']=='PCB':
            ...
            if hyperparameters['circle']:
                ...
                ff_a_circle = pp_a[0][0]
                ff_b_circle = pp_b[0][0]

                f_a_recon_circle = p_a_recon[0][0]
                f_b_recon_circle = p_b_recon[0][0]

                fnrom_f_a_circle = torch.norm(f_a_circle, p=2, dim=1, keepdim=True)
                fnrom_f_b_circle = torch.norm(f_b_circle, p=2, dim=1, keepdim=True)

                fnrom_s_a_circle = torch.norm(s_a_circle, p=2, dim=1, keepdim=True)
                fnrom_s_b_circle = torch.norm(s_b_circle, p=2, dim=1, keepdim=True)

                fnrom_ff_a_circle = torch.norm(ff_a_circle, p=2, dim=1, keepdim=True)
                fnrom_ff_b_circle = torch.norm(ff_b_circle, p=2, dim=1, keepdim=True)

                fnrom_f_a_recon_circle = torch.norm(f_a_recon_circle, p=2, dim=1, keepdim=True)
                fnrom_f_b_recon_circle = torch.norm(f_b_recon_circle, p=2, dim=1, keepdim=True)


                f_a_circle = f_a_circle.div(fnrom_f_a_circle.expand_as(f_a_circle))

                f_b_circle = f_b_circle.div(fnrom_f_b_circle.expand_as(f_b_circle))

                s_a_circle = s_a_circle.div(fnrom_s_a_circle.expand_as(s_a_circle))
                s_b_circle = s_b_circle.div(fnrom_s_b_circle.expand_as(s_b_circle))

                ff_a_circle = ff_a_circle.div(fnrom_ff_a_circle.expand_as(ff_a_circle))
                ff_b_circle = ff_b_circle.div(fnrom_ff_b_circle.expand_as(ff_b_circle))

                f_a_recon_circle = f_a_recon_circle.div(fnrom_f_a_recon_circle.expand_as(f_a_recon_circle))
                f_b_recon_circle = f_b_recon_circle.div(fnrom_f_b_recon_circle.expand_as(f_b_recon_circle))

                self.loss_id = self.id_criterion(p_a[0][1], l_a) \
                               + self.criterion_circle(*convert_label_to_similarity(f_a_circle, l_a))/now_batch_size \
                               + self.id_criterion(p_b[0][1], l_b) \
                               + self.criterion_circle(*convert_label_to_similarity(f_b_circle, l_b))/now_batch_size \
                               + weight_B * (self.id_criterion(p_a[1][1], l_a)
                               + self.criterion_circle(*convert_label_to_similarity(s_a_circle, l_a))/now_batch_size
                               + self.id_criterion(p_b[1][1], l_b)
                               + self.criterion_circle(*convert_label_to_similarity(s_b_circle, l_b))/now_batch_size)


                self.loss_pid = self.id_criterion(pp_a[0][1], l_a) \
                                + self.criterion_circle(*convert_label_to_similarity(ff_a_circle, l_a))/now_batch_size \
                                + self.id_criterion(pp_b[0][1], l_b) \
                                + self.criterion_circle(*convert_label_to_similarity(ff_b_circle, l_b))/now_batch_size # + weight_B * ( self.id_criterion(pp_a[1], l_a) + self.id_criterion(pp_b[1], l_b) )

                self.loss_gen_recon_id = self.id_criterion(p_a_recon[0][1], l_a)\
                                         + self.criterion_circle(*convert_label_to_similarity(f_a_recon_circle, l_a))/now_batch_size \
                                         + self.id_criterion(p_b_recon[0][1], l_b) \
                                         + self.criterion_circle(*convert_label_to_similarity(f_b_recon_circle, l_b)) / now_batch_size
            else:
                # normal loss
                # pa[0] is coarse code
                # pa[1] is fine code
                ...
                self.loss_trip_id = (self.triplet.forward(f_a, fp_a, f_b) + self.triplet.forward(f_b, fp_b, f_a)) if hyperparameters['trip_id_w'] > 0 else 0
                ...
                self.loss_trip_pid = (self.triplet.forward(fp_a, f_a, f_b) + self.triplet.forward(fp_b, f_b, f_a)) if hyperparameters['trip_pid_w'] > 0 else 0

                self.loss_cid = (self.id_criterion(pc_a[0], l_a) + self.id_criterion(pc_b[0], l_b)) if hyperparameters['cid_w'] > 0 else 0
            ...
            self.loss_trip_id = (self.triplet.forward(f_a, fp_a, f_b) + self.triplet.forward(f_b, fp_b, f_a)) if \
            hyperparameters['trip_id_w'] > 0 else 0
            ...
            self.loss_trip_pid = (self.triplet.forward(fp_a, f_a, f_b) + self.triplet.forward(fp_b, f_b, f_a)) if \
            hyperparameters['trip_pid_w'] > 0 else 0

            self.loss_cid = (self.id_criterion(pc_a[0], l_a) + self.id_criterion(pc_b[0], l_b)) if hyperparameters[
                                                                                                       'cid_w'] > 0 else 0

            self.loss_gen_recon_id = self.id_criterion(p_a_recon[0], l_a) + self.id_criterion(p_b_recon[0], l_b)

            self.loss_causal = self.id_criterion(feature_a, l_a) + self.id_criterion(feature_b, l_b) + \
                               self.id_criterion(causal_a, l_a) + self.id_criterion(causal_b, l_b) if hyperparameters['cau_w'] > 0 else 0

            self.loss_causal_pos = self.id_criterion(featurep_a, l_a) + self.id_criterion(featurep_b, l_b) + \
                               self.id_criterion(causalp_a, l_a) + self.id_criterion(causalp_b, l_b) if hyperparameters['cau_pw'] > 0 else 0

            self.loss_causal_cloth = self.id_criterion(featurec_a, l_a) + self.id_criterion(featurec_b, l_b) + \
                                   self.id_criterion(causalc_a, l_a) + self.id_criterion(causalc_b, l_b) if hyperparameters['cau_cw'] > 0 else 0

            self.loss_causal_recon = self.id_criterion(feature_a_recon, l_a) + self.id_criterion(feature_b_recon, l_b) + \
                                   self.id_criterion(causal_a_recon, l_a) + self.id_criterion(causal_b_recon, l_b) if hyperparameters['cau_recon_w'] > 0 else 0
        ...
        self.loss_gen_total = hyperparameters['gan_w'] * self.loss_gen_adv_a + \
                              ...
                              hyperparameters['cid_w'] * self.loss_cid + \
                              hyperparameters['trip_id_w'] * self.loss_trip_id + \
                              hyperparameters['trip_pid_w'] * self.loss_trip_pid + \
                              hyperparameters['recon_id_w'] * self.loss_gen_recon_id + \
                              hyperparameters['vgg_w'] * self.loss_gen_vgg_a + \
                              hyperparameters['vgg_w'] * self.loss_gen_vgg_b + \
                              hyperparameters['teacher_w'] * self.loss_teacher + \
                              hyperparameters['cau_w'] * self.loss_causal + \
                              hyperparameters['cau_pw'] * self.loss_causal_pos + \
                              hyperparameters['cau_cw'] * self.loss_causal_cloth + \
                              hyperparameters['cau_recon_w'] * self.loss_causal_recon
        ...
        accury_a = (p_a[0].max(1)[1] == self.gt_a).float().mean()
        accury_b = (p_b[0].max(1)[1] == self.gt_b).float().mean()

        accury_all_a = (feature_a.max(1)[1] == self.gt_a).float().mean()
        accury_all_b = (feature_b.max(1)[1] == self.gt_b).float().mean()

        accury_causal_a = (causal_a.max(1)[1] == self.gt_a).float().mean()
        accury_causal_b = (causal_b.max(1)[1] == self.gt_b).float().mean()

        print("L_total: %.4f, L_gan: %.4f,  Lx: %.4f, Lxp: %.4f, Lrecycle:%.4f, Lf: %.4f, Ls: %.4f, Recon-id: %.4f, id: %.4f, pid:%.4f, cid:%.4f, trip_id:%.4f, trip_pid:%.4f, teacher: %.4f, causal-id: %.4f, causal-pid: %.4f, causal-cid: %.4f, causal-recon-id: %.4f, ACC-a: %.4f, ACC-b: %.4f, ACC-all-a: %.4f, ACC-all-b: %.4f, ACC-causal-a: %.4f, ACC-causal-b: %.4f"%( self.loss_gen_total, \
                                                        ...
                                                        hyperparameters['cau_cw'] * self.loss_causal_cloth , \
                                                        hyperparameters['cau_recon_w'] * self.loss_causal_recon, \
                                                        accury_a, accury_b, accury_all_a, accury_all_b, accury_causal_a, accury_causal_b\
                                                    ))
        # if iteration == 100:
        #     assert 1!=1
        return accury_a, accury_b, accury_all_a, accury_all_b, accury_causal_a, accury_causal_b
    ...
    def sample(self, x_a, x_b):
        ...
        for i in range(x_a.size(0)):
            ...
            if self.ACID:
                #
                f_a, _, _, _ = self.id_a(scale2(x_a[i].unsqueeze(0)), self.gt_a, False)
                f_b, _, _, _ = self.id_b(scale2(x_b[i].unsqueeze(0)), self.gt_b, False)
            else:
            ...
            if self.ACID:
                #
                f_a_recon, _, _, _ = self.id_a(scale2(x_ba), self.gt_a, False)
                f_b_recon, _, _, _ = self.id_b(scale2(x_ab), self.gt_b, False)
            else:
        ...
        # a图， 自重建的a图， 重建图提取再重建的a图， a结构b外观的图
    ...
    def dis_update(self, x_ab, x_ba, x_a, x_b, hyperparameters, num_gpu):
        ...
        print("DLoss: %.4f"%self.loss_dis_total, "Reg: %.4f"%(reg_a+reg_b), "Time:", strftime('%Y-%m-%d %H:%M:%S', localtime()))

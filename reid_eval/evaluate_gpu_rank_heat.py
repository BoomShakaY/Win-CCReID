...
for j in range(len(alpha)):
    ...
    if j == 1:
        dist_file = opt.mat_saved_path + '/result_rank.json'
        results_dict = dict()
...
if opt.cloth:
    command = 'python show_ranks.py --cloth --name ' + opt.name + ' --mat_saved_path ' + opt.mat_saved_path
    os.system(command)
elif opt.mix_cloth:
    command = 'python show_ranks.py --mix_cloth --name ' + opt.name + ' --mat_saved_path ' + opt.mat_saved_path
    os.system(command)
else:
    # standard
    command = 'python show_ranks.py --name ' + opt.name + ' --mat_saved_path ' + opt.mat_saved_path
    os.system(command)
